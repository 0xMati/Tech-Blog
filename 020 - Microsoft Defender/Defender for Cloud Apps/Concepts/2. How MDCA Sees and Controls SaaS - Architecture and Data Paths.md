...
The same file download can produce three separate evidence chains:

1. Defender for Endpoint reports the network connection.
2. The provider API reports the file operation.
3. Conditional Access App Control observes or blocks the browser action.
...
The unified portal hides different owners, schemas, clocks, and failure modes. If evidence is missing, ask which path was supposed to produce it.
...
Discovery observes traffic **around** the app, not business actions **inside** it. It can identify application usage, users, devices, IP addresses, volume, and risk when the source provides those fields. Filenames, permission changes, OAuth grants, and administrative settings remain hidden unless another path exposes them.

A 400 MB upload is useful evidence. It is not a filename or proof of exfiltration.
...
Uploaded logs can take from minutes to hours to process, depending on volume. Cloud Discovery data is batch-analyzed several times per day. When the MDE integration is first enabled, initial data can take up to two hours to appear.
...
The **Governance log** records what MDCA attempted and the returned status. Verify the resulting state in the SaaS platform and its audit log. A policy deciding to suspend an account and the provider actually suspending it are two separate events.
...
Microsoft Edge for Business can use in-browser protection for supported scenarios, avoiding the visible reverse-proxy URL rewrite. As of September 2026, Microsoft documents that experience as Preview.
