...
> 🎯 **TL;DR** — Microsoft Defender for Cloud Apps (MDCA) is not one sensor and it is not just a Cloud Access Security Broker (CASB). It combines traffic discovery, SaaS APIs, behavioral detections, OAuth app governance, security posture assessments, and browser session controls. Each capability sees a different slice of reality. Understanding those boundaries is the difference between operating MDCA and merely owning a license.
...
The easiest way to misunderstand MDCA is to imagine a magic cloud sensor watching everything. There is no magic sensor.

MDCA assembles visibility from three fundamentally different paths:
...
| **Microsoft Purview** | Data classification, labeling, DLP, and compliance | Supplies data context and protection mechanisms used in supported MDCA scenarios. |
