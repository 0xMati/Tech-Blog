...
> **⚠️ This is least-privilege and audit clarity — not a security boundary (inside Tier 0).** `T0-N2-ADFS` and `T0-N2-PKI` are both Tier 0 *precisely because compromising either one compromises the whole forest*: AD CS abuse (ESC / certificate forgery) yields Domain Admin, and theft of the ADFS token-signing key lets an attacker impersonate anyone. The firewall *does* block a direct network hop (RDP/WinRM) from an ADFS host to a PKI host — but that is not where the danger is. Compromising one T0 silo makes the attacker an **identity authority for the forest**, so they reach their objective **without ever using the filtered network path**: a stolen ADFS token-signing key forges tokens as Domain Admin (then DCSync, golden ticket…); an AD CS ESC abuse forges a certificate that authenticates as Domain Admin. This shared blast radius is **invariant — it exists with or without IPsec segmentation**, because it comes from what a Tier 0 silo *is*, not from how the network is wired. And precisely because the danger is unaffected by segmentation, **adding IPsec cannot reduce it** — it only changes which path the legitimate admin packets take. Containment fails not because the firewall lets traffic through, but because forest dominance bypasses the network path entirely. Inside Tier 0, host isolation therefore buys **least privilege, cleaner audit trails, and operational hygiene** (an ADFS admin's daily mistakes don't land on PKI hosts) — calling it "containment" would be security theater.
