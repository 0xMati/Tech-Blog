...
> **⚠️ This is least-privilege and audit clarity — not a security boundary (inside Tier 0).** A tier is a **classification of sensitivity**, not a statement that its members can compromise each other. What makes a system Tier 0 is that it **controls the identity of the forest** — and that control *is* the shared asset. `T0-N2-ADFS` and `T0-N2-PKI` are both Tier 0 for exactly that reason: stealing the ADFS token-signing key lets an attacker forge tokens as anyone (including Domain Admin → DCSync, golden ticket…), and AD CS abuse (ESC / certificate forgery) lets them mint a certificate that authenticates as Domain Admin. Either one yields the whole forest. The firewall *does* block a direct network hop (RDP/WinRM) from an ADFS host to a PKI host — but that is not where the danger is: once an attacker owns one T0 silo they are an **identity authority for the forest** and reach any objective **without ever using the filtered network path**. This shared blast radius is **invariant — it exists with or without IPsec segmentation**, because it comes from what a Tier 0 silo *is*, not from how the network is wired. And precisely because the danger is unaffected by segmentation, **adding IPsec cannot reduce it** — it only changes which path the legitimate admin packets take. Inside Tier 0, host isolation therefore buys **least privilege, cleaner audit trails, and operational hygiene** (an ADFS admin's daily mistakes don't land on PKI hosts) — calling it "containment" would be security theater.

**Where it becomes real containment: Tier 1.** Tier 1 members do **not** share an asset by definition — they are independent business systems that merely sit in the same sensitivity class. Compromising a SQL server makes the attacker admin *of that SQL server*, not of Exchange; their blast radii are disjoint. There, blocking `T1-N2-SQL` from reaching an Exchange host genuinely confines an incident — exactly the boundary this document already states ("N2 group for Exchange cannot WinRM to SQL"). So the example that *justifies* the mechanism is T1-SQL/Exchange; ADFS/PKI is the T0 *hygiene* case.
>
> **⚠️ Caveat — "disjoint" assumes no hidden shared asset.** Two T1 platforms only have separate blast radii if they share neither a **common substrate** (the same virtualization host, backup system, or service account) nor a **residual privilege path to AD**. A hypervisor hosting both VMs sits *below* them, so owning it owns both — it is a blast-radius concentrator, not a peer (and one hosting any T0 guest becomes T0 itself). Likewise, on-prem Exchange with default `Exchange Windows Permissions` has a documented escalation to Domain Admin (write-DACL on the domain object → DCSync); until that is hardened, Exchange is not cleanly T1. Where such a shared asset exists, segmentation again becomes hygiene rather than containment.
