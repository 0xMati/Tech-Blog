...
| Kdcsvc **202 / 207** firing on a service account | Stale-key trap — bitmask declares AES but `supplementalCredentials` has no AES key | Rotate the service account password immediately — re-check System log on next request |
| Kdcsvc **201 / 206** firing from a specific source IP | Client-side enctype lock (krb5.conf, keytab, vendor appliance) | Fix the client; as a temporary workaround set service attribute to `0x24` (RC4 + AES session keys) |
| Kdcsvc **203 / 204 / 208 / 209** errors after April 2026 | Phase-2 enforcement is blocking auth — same root cause as the warnings, but production is now broken | Triage the account, then rotate password / fix client / set `0x24` per case. **Do not** roll the registry back to `1` for the whole domain |
