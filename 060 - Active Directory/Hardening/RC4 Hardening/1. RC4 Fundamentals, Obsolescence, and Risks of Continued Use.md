...
**First, what this is *not* about.** When people say "RC4 is broken", they usually mean the academic attacks: **RC4 NOMORE** (2015, recovers HTTPS cookies), **FMS** (2001, killed WEP Wi-Fi), and the various biases on the RC4 keystream. Those attacks need **millions of ciphertexts encrypted with the same key** to work. A Kerberos ticket is **one** ciphertext, used once. Those attacks do not apply here. Forget them.

**The real problem is upstream of the cipher: it's the KDF.**
...
| **AES128 / AES256** | `K = PBKDF2-HMAC-SHA1(password, salt, 4096 iterations)`, salt = `<REALM><principalName>` | **Yes** | ~200 **thousand** guesses/sec — same hardware |
...
Kerberoasting and AS-REP roasting are the two attacks RC4 *enables directly*. A few other paths use RC4 as a cost-reducer rather than a primary vector — worth naming once so they are not forgotten, but each has a more focused home elsewhere in the series:

- **Trust ticket abuse across forests/domains** — RC4 referral tickets combined with stale trust passwords keep cross-realm hops cheap for an attacker. Treated in depth in the [trust hardening annex](../Hardening%20Kerberos%20Encryption%20on%20AD%20Trusts/Hardening%20Kerberos%20Encryption%20on%20AD%20Trusts.md) and in article 3.
- **Encryption downgrade** — when `msDS-SupportedEncryptionTypes` still contains `0x04` (the `0x1C` pattern decoded in §6), legacy clients or crafted requests can keep negotiating RC4 even on otherwise-AES-clean estates. The bitmask decoder in §6 covers what to look for.
- **Post-cracking lateral movement** — once a service password from a Kerberoasting attack is recovered, it gets reused like any stolen password (overpass-the-hash, runas, RDP into legacy hosts). Not RC4-specific from that point on, but RC4 was the cheap on-ramp.

In all three cases, removing RC4 doesn't kill the attack — it removes the accelerator. That alone is worth the migration cost.
