...
>
> These two can disagree — and this is the heart of the November 2022 KDC change (KB5021131): the session key now defaults to AES even when the envelope is RC4, which closes the largest Kerberoasting blind spot in legacy estates. We will come back to this distinction in §3's deep dives, in the runtime evidence section (§4 *4768/4769*), and again when discussing trust hardening.
...
## 4. Static config lies, runtime doesn't 👀

§3 explained *how* Kerberos works and *where* RC4 sits inside the cryptography. Before talking about why RC4 is dangerous, one operational point has to land — because it is the assumption every later section depends on:

> **A clean static configuration does not prove RC4 is gone. Only live ticket telemetry does.**

RC4 usually survives by accident, not by design. The classic failure mode looks like this:

- Service account `svc_sql_legacy` has `msDS-SupportedEncryptionTypes = 0x1C` and a password that has not rotated in years.
- DCs are patched, the encryption-types GPO looks modern, the inventory dashboard is green.
- And `4769` still emits RC4 tickets every minute for `MSSQLSvc/...` SPNs.
...
The config-vs-runtime gap is where most RC4 projects fail late — typically at cutover, when an AES-only enforcement breaks production because nobody validated the runtime first (the outage walkthrough in §7 plays this out hour by hour).

This article stays at the *why* level. The full **`4768`/`4769` field map** — which fields to read first, what changed with the January 2025 cumulative update, and how to write the inventory queries — lives in **article 2** (*Legacy Dependency Mapping and Technical Inventory*), where it has its proper home alongside the LDAP/AD inventory queries and the trust-side audit. Just retain three rules of thumb here:

- **`4769` + `Ticket Encryption Type = RC4`** = an RC4 service ticket left the wire. Real attack surface.
- **`4768` showing RC4 capability for the same client repeatedly** = an RC4-only requester. Hunt the device, not the account.
- **Static `msDS-SupportedEncryptionTypes` is necessary but not sufficient.** Treat it as a hypothesis, confirm it on `4768`/`4769`.

## 5. Why RC4 is dangerous: weak crypto + the attacks it enables 🧨

Now that the runtime view is set, here is why a single RC4 ticket leaking onto the wire is enough to matter.
...
The mental model for the rest of this section:
...
- RC4 is a 1990s stream cipher and modern crypto standards have moved on — keeping it enabled in 2026 is like leaving a copy of your house key under the doormat "just in case".
- The danger isn't theoretical: weak crypto plus a low-privilege LDAP read is enough to silently crack service or user passwords on attacker hardware, with zero traffic back to your AD.
- Most real-world AD attack chains start at the weakest crypto path available. If RC4 is present anywhere, that is where offensive effort goes first.
...
- RC4 tickets dramatically reduce the cost of offline cracking against service and user accounts.
- Microsoft hardening direction is clearly AES-first, then AES-only.

The next subsections build the danger argument in three steps: **why** the crypto is weak (KDF cost), **where** that weakness is exposed (which envelopes are attackable), and **how** an attacker actually exploits it (two walkthroughs).

### Why RC4 cracks so much faster than AES (the real reason)

It isn't about RC4 being a "broken cipher" in the academic sense — the famous RC4 biases (RC4 NOMORE, FMS, etc.) need millions of ciphertexts encrypted with the same key and **do not apply to a single Kerberos ticket**. The real throughput gap is in how the long-term key is derived from the password:
...
- A strong, random 25+ character password is computationally infeasible against either — which is why MITRE M1027 (long service passwords) exists as the backstop to M1041 (prefer AES over RC4).
...
- Both RC4 and AES envelopes can be Kerberoasted — hashcat supports `13100` (RC4 TGS-REP), `19600` (AES128 TGS-REP), `19700` (AES256 TGS-REP). The difference is **throughput**, not feasibility (see §5 for why RC4 is orders of magnitude faster).
...
## 6. RC4 in larger attack chains 🎯

§5 covered the two **direct** offline-cracking attacks RC4 enables: Kerberoasting and AS-REP roasting. RC4 also lubricates other attack chains where it isn't the primary vector but still lowers attacker cost — trust boundaries, downgrade negotiation, and post-cracking pivots.
...
| Credential theft + lateral movement acceleration | Uses cracked service credentials (from §5) to pivot | RC4-compatible legacy islands are often the easiest pivot points in mature AD | Same identities repeatedly involved in RC4 tickets + lateral auth spikes |
...
## 7. Operational risks of unplanned RC4 disablement ⚙️
...
## 8. What "ready for RC4 removal" means technically ✅
...
| **`0x04` / `0x18` / `0x1C`** | Common bitmask values: RC4-only / AES-only / AES+RC4. See §8 for full decoding. |
