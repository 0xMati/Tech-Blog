...
**Envelope structure** — what a Kerberos ticket actually looks like on the wire:

```text
┌─────────────────────────────────────────────────────────────┐
│  TICKET (opaque to the client, readable only by the target) │
│                                                             │
│   ┌──────────────────────────────────────────────────┐      │
│   │  EncTicketPart — sealed with K_target            │      │
│   │  (K_krbtgt for a TGT, K_svc for a service ticket)│      │
│   │                                                  │      │
│   │  Inside:                                         │      │
│   │   • Session key (K_c,tgs or K_c,s)               │      │
│   │   • Client identity (PAC, SIDs, group claims)    │      │
│   │   • Validity period, flags                       │      │
│   └──────────────────────────────────────────────────┘      │
└─────────────────────────────────────────────────────────────┘
        +
┌─────────────────────────────────────────────────────────────┐
│  enc-part — sealed with the requester's key                 │
│  (K_user for AS-REP, K_c,tgs for TGS-REP)                   │
│  Inside: a copy of the same session key, for the client.    │
└─────────────────────────────────────────────────────────────┘
```

> 🧠 **Two envelopes, one session key.** The KDC always seals the session key **twice**: once for the target (inside the ticket) and once for the requester (in the enc-part). That is how both ends end up with the same shared secret without it ever travelling in clear.
...
| **`K_c,tgs`** | Short-lived session key between client and KDC | Client + KDC (via the TGT) | TGT lifetime (~10h default) | Seals TGS-REQ authenticator and TGS-REP enc-part |
| **`K_c,s`** | Short-lived session key between client and target service | Client + service (via the service ticket) | Service ticket lifetime (~10h default) | Seals AP-REQ authenticator and any subsequent app-level Kerberos integrity/confidentiality |
