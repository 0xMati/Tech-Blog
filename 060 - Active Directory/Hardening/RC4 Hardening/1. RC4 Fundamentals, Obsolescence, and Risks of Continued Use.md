...
>
> **At authentication time — when does the KDF actually run?** The KDF is **not** re-played on every Kerberos message. Here is what actually happens in practice:
>
> | Moment | Is the KDF re-run? | Why |
> | --- | --- | --- |
> | User types their password at logon (Winlogon, RDP, `runas`) | ✅ Yes, on every logon | The client only holds the cleartext password for a few milliseconds; it must derive the keys from it to send the AS-REQ |
> | Service starts up with a stored credential (service account, scheduled task, gMSA) | ✅ Yes, once at startup | LSA pulls the password from the LSA Secrets store, derives the keys, places them in LSASS |
> | TGT renewal, TGS request, ticket presentation… | ❌ No | The keys are **already cached in LSASS** since the initial logon. Every subsequent exchange reuses them as-is |
> | KDC validating an AS-REQ or issuing a TGS | ❌ Never | The KDC **reads the keys directly** from `NTDS.dit` (`unicodePwd` for RC4, `supplementalCredentials` for AES). It never sees the cleartext password |
>
> Three takeaways that matter:
>
> - **The cleartext password only exists briefly on the client**, just long enough for the derivation. It is then wiped from memory; only the long-term keys remain in LSASS for the duration of the session. (That is exactly why `mimikatz sekurlsa::logonpasswords` extracts NT hashes and Kerberos keys, but only recovers the cleartext password in specific cases such as WDigest being enabled.)
> - **The password never travels on the wire.** No Kerberos message contains the password — or even the long-term keys themselves. Only ciphertext encrypted with them does.
> - **Both ends end up with the same key without ever exchanging it**: the client by local re-derivation, the KDC by reading it from the database. That is what makes Kerberos a *shared-secret* protocol.
>
