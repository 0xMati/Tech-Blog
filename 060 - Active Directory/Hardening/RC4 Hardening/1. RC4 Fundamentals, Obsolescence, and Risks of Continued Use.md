...
Before diving into the IDs, **two shorthands** that show up everywhere in this sub-section:

- **`msds-SET`** = `msDS-SupportedEncryptionTypes` — the **per-account** AD attribute (user, computer, gMSA, TDO). It says *"these are the etypes the KDC is allowed to use for this specific principal"*.
- **`DDSET`** = `DefaultDomainSupportedEncTypes` — the **per-DC** registry value (`HKLM\SYSTEM\CurrentControlSet\Services\Kdc`). It's the **fallback** the KDC uses when `msds-SET` is empty or `0` — whichever DC handles the request applies its own local value.

These two values are the "policy" — what AD **declares is allowed**. Sitting next to the policy is the **physical reality** of AES keys in `supplementalCredentials`, which can be missing even when policy permits AES (the §2 🔑 trap).
...
1. **What does policy say?** Is the service's `msds-SET` empty (KDC falls back to the DC's `DDSET`), or explicitly `0x18` (AES-only)?
...
```
   ...
   ─────────────────┴────────────────────────┴────────────────────────┘
                ...
                + 1 pure-hygiene event, off-grid:
                205 → a DC has DDSET explicitly insecure
```
...
| Phase 1 (warning) | Phase 2 (error) | Trigger | What it actually means |
| --- | --- | --- | --- |
| **206** | **208** | The **client** advertises only RC4 *and* the service has `msds-SET = 0x18` (or DC `DDSET = 0x18`) | Same symptoms as 201/203, but this time AES-only is **explicitly assumed**. From a triage standpoint that's reassuring: someone made the effort to set `0x18`, so the attribute is correct; the problem is purely client-side. **Action**: same — fix the client, or temporary `0x24` lifeline on the service. |
| **207** | **209** | The **service** has no AES keys *and* `msds-SET = 0x18` (or DC `DDSET = 0x18`) | **The 🪤 trap in its most painful form**: the attribute declares *"I do AES only"*, but disk reality says *"I have zero AES keys"*. Policy is lying to itself. The KDC has no option and refuses. **Action**: rotate the password immediately — this aligns reality (keys on disk) with declaration (`0x18`). This is exactly the scenario the §6 reminder is meant to prevent, playing out for real. |

##### Off-grid: a single hygiene event (never blocking)

| Event | When it fires | What it tells you |
| --- | --- | --- |
| **205** | A DC has `DefaultDomainSupportedEncTypes` **explicitly set** to a value containing anything other than AES-SHA1 (i.e. RC4 or DES bits set). Logged **once at every Kdcsvc startup**. | An admin has deliberately put RC4 back into that DC's `DDSET`. Microsoft will **never block** this config — explicit admin intent always overrides Microsoft defaults. But the event fires to flag the DC as less secure than the rest of the fleet. **Action**: ask whether the override is still justified. If not, clear the value — the new `0x18` default takes back over automatically. |
...
- Event is **205** → **DC-side** problem (registry override) → fix = **review the override**.
...
| **205 warning at KDCSVC startup** | 1 or 2 | A DC has explicit insecure DDSET configured | Audit the registry on that DC and align with the rest of the fleet |
