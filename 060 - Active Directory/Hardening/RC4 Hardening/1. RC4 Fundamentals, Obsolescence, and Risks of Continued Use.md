...
## 0.5. Two kinds of keys, and why attackers only care about one 🔑
...
> 🔑 **What "long-term key" actually means.** Active Directory never stores passwords in cleartext (with the rare and discouraged exception of *reversible encryption*). When a password is set on an account, AD does **two things in one shot**:
>
> **1. Compute the keys** — the password is run through one or more Key Derivation Functions (KDFs). Each Kerberos enctype has its own KDF:
>
> | Enctype | KDF applied to the password | Result |
> | --- | --- | --- |
> | RC4-HMAC | `MD4(UTF-16LE(password))` — one single MD4 pass, no salt, no iterations | A 128-bit key. **This key is exactly the NT hash.** RC4-HMAC then uses it as-is to seal Kerberos tickets. |
> | AES128-CTS-HMAC-SHA1-96 | `PBKDF2(password + salt, 4096 iterations, SHA-1)` per RFC 3962. AD salt = `<REALM><principalName>` | A 128-bit key, used by AES128 to seal tickets. |
> | AES256-CTS-HMAC-SHA1-96 | Same PBKDF2, same salt, same iterations, longer output | A 256-bit key, used by AES256 to seal tickets. |
>
> **2. Store the keys** in the directory database (`NTDS.dit`):
>
> - The **NT hash** sits behind the LDAP attribute `unicodePwd` (write-only — you can never read it back through LDAP or the GUI). It is exposed only through directory replication (`DS-Replication-Get-Changes-All` — the right DCSync abuses) or local DC access (`ntdsutil`, `DSInternals`).
> - The **AES128 and AES256 keys** sit in the attribute `supplementalCredentials`, in a sub-blob called `Primary:Kerberos-Newer-Keys`. Same access restrictions.
>
> > 🚩 **Two crucial properties of this storage:**
> >
> > - **The NT hash is ALWAYS present on any account that has a password.** Setting one writes it; there is no "disable NT hash" switch. Windows still needs it for NTLM, cached interactive logon, and a few legacy protocols — completely independently of whether RC4 is allowed for Kerberos.
> > - **AES keys are CONDITIONAL.** They are written only when the password was set or changed on a domain at functional level 2008 or higher (and on a DC running 2008+). An account whose password has not been touched since pre-2008 has **no AES keys at all** in `supplementalCredentials`, even if you set `msDS-SupportedEncryptionTypes` to `0x18`. Setting the attribute does not generate keys — only a password rotation does. **This is the *stale-key trap*** explored in Article 2 and the trust annex.
>
> **3. At authentication time, the keys are re-derived on the fly by the client** — when a user types their password into Winlogon (or a service stack provides it from a credential vault), the client runs the same KDFs locally to obtain the same long-term keys, then uses them to encrypt/decrypt Kerberos messages. The key never travels on the wire; only ciphertext encrypted with it does. This is what makes Kerberos a "shared secret" protocol: both ends derive the same key from the same password, independently.
>
> **And this is exactly why RC4 is dangerous.** A ticket sealed with RC4-HMAC is sealed with the NT hash itself. Cracking that ticket offline (Kerberoasting) does not just recover "a Kerberos key" — it recovers the **NT hash directly**, which is the same secret reused by NTLM, cached credentials, and every other Windows authentication path that touches that account. Pass-the-hash, full account compromise, no further cracking needed.
>
> AES doesn't change *what* is protected (the same password sits behind both keys). It changes *how slow* the brute-force pipeline is — one MD4 per candidate password for RC4, versus 4096 SHA-1 rounds plus a per-account salt for AES. That salt also defeats precomputed rainbow tables, which is a separate and equally important defense.

What this means in plain English:

- A ticket is a **sealed envelope**. The seal is always made with a long-term key (`K_krbtgt` for the TGT, `K_svc` for a service ticket). Inside the envelope sits a session key plus identity information.
- "Cracking a ticket" really means: brute-forcing the long-term key that sealed the envelope, by trying passwords offline until decryption produces sensible output.
- The cipher (RC4 or AES) decides **how fast** the brute force runs. It does not change *what* is being attacked.
