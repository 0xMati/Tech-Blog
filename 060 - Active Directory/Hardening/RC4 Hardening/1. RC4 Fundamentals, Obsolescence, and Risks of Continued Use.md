...
> 🔑 **"Long-term key" = `KDF(password)`.** Active Directory never stores passwords in cleartext (unless *reversible encryption* is explicitly enabled, which is discouraged) — it stores keys derived from the password through a Key Derivation Function. Each enctype has its own KDF:
>
> | Enctype | Derivation | Result |
> | --- | --- | --- |
> | RC4-HMAC | `MD4(UTF-16LE(password))` — single MD4 pass, no salt, no iteration count | 128-bit key — **this is exactly the NT hash** |
> | AES128-CTS-HMAC-SHA1-96 | `PBKDF2(password + salt, 4096 iterations, SHA-1)` (RFC 3962). Default AD salt: `<REALM><principalName>` | 128-bit key |
> | AES256-CTS-HMAC-SHA1-96 | Same PBKDF2 with the same salt and iteration count, longer output | 256-bit key |
>
> Where the keys live on disk:
>
> - **NT hash** — behind the write-only LDAP attribute `unicodePwd`, in the `NTDS.dit` database. Cannot be read via GUI or normal LDAP; only exposed through directory replication (`DS-Replication-Get-Changes-All` — what DCSync abuses) or local DC access (`ntdsutil`, `DSInternals`). The NT hash is **always present** on any account that has a password — Windows still needs it for NTLM and a handful of legacy protocols, regardless of whether RC4 is enabled for Kerberos.
> - **AES128 and AES256 keys** — in the `supplementalCredentials` attribute, under the `Primary:Kerberos-Newer-Keys` sub-package. Same access restrictions. Crucially, **AES keys are conditional**: they only exist if the password was set or changed on a Windows version that supports AES (Server 2008+ / DFL ≥ 2008). An account whose password has not been touched since pre-2008 has **no AES keys at all**, even if `msDS-SupportedEncryptionTypes` is set to `0x18` — that is the *stale-key trap* explored in Article 2 and the trust annex. Only a password rotation materializes them.
>
> The owner of the account re-derives these keys from the password every time they authenticate. **This is what "long-term key" means throughout this article — and it explains why RC4 is dangerous: cracking an RC4-protected ticket recovers the NT hash, which is the password derivative, which is the same secret used by every other Windows authentication protocol that ever touches that account.** AES doesn't change *what* is being protected — it changes *how slow* the brute-force pipeline is (one MD4 vs 4096 SHA-1 rounds per candidate password, with a salt that defeats rainbow tables).
