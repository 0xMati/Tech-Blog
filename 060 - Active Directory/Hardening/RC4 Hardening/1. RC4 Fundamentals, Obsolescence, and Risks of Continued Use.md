...
- Both RC4 and AES envelopes can be Kerberoasted — hashcat supports `13100` (RC4 TGS-REP), `19600` (AES128 TGS-REP), `19700` (AES256 TGS-REP). The difference is **throughput**, not feasibility (see §4 for why RC4 is orders of magnitude faster).
...
## 5. RC4 in larger attack chains 🎯

§4 covered the two **direct** offline-cracking attacks RC4 enables: Kerberoasting and AS-REP roasting. RC4 also lubricates other attack chains where it isn't the primary vector but still lowers attacker cost — trust boundaries, downgrade negotiation, and post-cracking pivots.
...
| Credential theft + lateral movement acceleration | Uses cracked service credentials (from §4) to pivot | RC4-compatible legacy islands are often the easiest pivot points in mature AD | Same identities repeatedly involved in RC4 tickets + lateral auth spikes |
...
## 6. Operational risks of unplanned RC4 disablement ⚙️
...
## 7. What "ready for RC4 removal" means technically ✅
...
| **`0x04` / `0x18` / `0x1C`** | Common bitmask values: RC4-only / AES-only / AES+RC4. See §7 for full decoding. |
