...
- **Directory posture** — `msDS-SupportedEncryptionTypes` reviewed on relevant principals (users, computers, gMSAs, service accounts).
- **KDC default posture** — `DefaultDomainSupportedEncTypes` registry value on DCs (where applicable).
...
| --- | --- | --- |
| `0x01` | `00000001` | DES-CBC-CRC (long-dead — flag for immediate removal if found) |
| `0x02` | `00000010` | DES-CBC-MD5 (long-dead — same) |
| `0x04` | `00000100` | RC4-HMAC |
| `0x08` | `00001000` | AES128-CTS-HMAC-SHA1-96 |
| `0x10` | `00010000` | AES256-CTS-HMAC-SHA1-96 |

A real value is a **bitwise OR** of the enabled flags (since the bits don't overlap, OR is just addition):

- `0x04` = `00000100` = **RC4 only**. Worst-case service account.
- `0x18` = `0x08 | 0x10` = `00011000` = **AES128 + AES256, no RC4**. The modern target state.
- `0x1C` = `0x04 | 0x08 | 0x10` = `00011100` = **AES + RC4 still allowed**. Looks AES-capable, but RC4 is still on the menu — the most common downgrade-capable state in the wild.
- `0x00` (or attribute missing) = **default policy applies**. On most domains that still includes RC4 today.

AD tooling (PowerShell, ADUC, ADSI Edit) often surfaces these as **decimal** rather than hex — convert mentally: `4` → `0x04`, `24` → `0x18`, `28` → `0x1C`, `0` → default.
