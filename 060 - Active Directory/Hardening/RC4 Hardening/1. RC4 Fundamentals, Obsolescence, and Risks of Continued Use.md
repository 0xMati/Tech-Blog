...
>
> **At authentication time**, the client re-derives the same keys locally from the password the user typed (or that a service stack pulled from a credential vault), and uses them to encrypt/decrypt Kerberos messages. The keys never travel on the wire — only ciphertext encrypted with them does. Both ends derive the same value from the same password, independently. That is what makes Kerberos a *shared-secret* protocol.
>
