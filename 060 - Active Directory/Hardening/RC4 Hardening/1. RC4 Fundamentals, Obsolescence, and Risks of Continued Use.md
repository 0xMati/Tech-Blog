...
| **Session key** | Generated randomly by the KDC for each ticket exchange. Never derived from a password. | Bound to ticket lifetime — **10 h by default in AD** (renewable up to 7 days), down to minutes on hardened tiers (PAW, JIT admin). | Not worth cracking. Even if recovered, expires within the ticket lifetime and is scoped to one specific exchange — orders of magnitude shorter-lived than the long-term key it sits next to. |
...
> 3. **Protect application traffic afterwards.** Once `AP-REQ` succeeds, GSSAPI / SSPI use the session key (or sub-keys derived from it) to sign and/or encrypt every subsequent message of the application protocol — this is how Kerberos protects an *entire* SMB or RPC session, not just the initial handshake. Concretely:
>
>    | Protocol | What the session key protects |
>    |---|---|
>    | **SMB 2/3** | Per-message integrity (SMB signing) and optional confidentiality (SMB encryption) on every read / write |
>    | **LDAP** | Signing and optional sealing of every LDAP request and response — defeats relay-then-modify attacks against AD |
>    | **RPC** | Per-call integrity or privacy (`RPC_C_AUTHN_LEVEL_PKT_INTEGRITY` / `_PRIVACY`) — used by AD replication (MS-DRSR), DCOM, SCM, MS-RPRN, etc. |
>    | **HTTP / WinRM** | Signing and integrity of WinRM and HTTP Negotiate messages when `AllowUnencrypted = false` |
>
>    The takeaway: **a weak session key weakens every byte of the application protocol** that rides on it — biased keystream and MD5 integrity if `Session Key Type: RC4-HMAC` shows up in `klist`, AES-CTS-HMAC-SHA1 if it does not.
>
...
> 🧠 **A *ticket* is one envelope, not two.** The ticket itself is a single sealed blob. What has *two* sealed parts is the **KDC response message** (`AS-REP` for the TGT step, `TGS-REP` for the service-ticket step) that delivers the ticket to the client:
>
> ```text
> AS-REP / TGS-REP message
> ├── Ticket     ──  sealed with K_target     ─→  only the target can open
> └── enc-part   ──  sealed with K_requester  ─→  only the client can open
>                                                 (carries the client's copy
>                                                  of the session key)
> ```
>
> Both halves contain the same random session key — that is how client and target end up sharing it without it ever traveling in cleartext. You will see that two-part structure spelled out in Deep Dive 1 below; the schema above is the ticket on its own, which is what travels onward in TGS-REQ and AP-REQ.
...
- **PKINIT / smartcard logon** — accounts that authenticate via certificate don't derive `K_user` from a password; the AS exchange uses a Diffie-Hellman key authenticated by the cert. AS-REP roasting becomes structurally impossible on those accounts, regardless of RC4. Treated alongside other compensating controls in **article 3 — *Compensating controls when RC4 cannot be removed immediately***.
- **FAST / Kerberos armoring** — wraps the AS exchange in an encrypted tunnel so the AS-REP `enc-part` is never directly observable on the wire. Treated in the same **article 3** section.
...
- **KDC default posture** — `DefaultDomainSupportedEncTypes` registry value on each DC (`HKLM\SYSTEM\CurrentControlSet\Services\Kdc`). This is the **fallback** the KDC applies when an account's `msDS-SupportedEncryptionTypes` is `0` or missing — i.e. when the directory says nothing. Not replicated; it's a per-DC registry value, so it has to be checked (and aligned) on every DC. Two distinct Microsoft changes touch this layer and are commonly conflated — keep them separate:
    - **November 2022 (KB5021131 / CVE-2022-37966)** changed the **session-key** default to AES even when the ticket envelope is RC4 (the anti-downgrade fix referenced in §2 and §4). It did **not** change the ticket envelope default — an unset account could still receive an RC4 envelope.
    - **April 2026 (KB5073381)** changed the **ticket envelope** default: when `msDS-SupportedEncryptionTypes` is unset, the effective `DefaultDomainSupportedEncTypes` flips to **AES-only (`0x18`)**. This is the change that finally retires implicit RC4. Detailed in the Kdcsvc 201-209 sub-section below.
...
### Triage cheat sheet: directory values vs event-log values

In day-to-day work you'll meet these hex values in **two very different contexts**, and the action you take depends on which:

- when running an **LDAP / PowerShell inventory** (article 2) you read `msDS-SupportedEncryptionTypes` on accounts — *"what is this account allowed to do?"* This tells you the **declarative state** and drives remediation work (rotate password, change attribute, etc.).
- when running a **SIEM / log query** on `4768` / `4769` events you read the `Ticket Encryption Type` field — *"what did the KDC actually issue, on this exact request?"* This tells you the **runtime reality** and drives detection / readiness validation.

Same hex literals, opposite namespaces. Keep the two tables below open while triaging.

**Directory attribute (`msDS-SupportedEncryptionTypes`) — what the account is *allowed* to do:**

| Value | Meaning | Engineering signal |
| --- | --- | --- |
| `0x04` (decimal `4`) | RC4 only | High-priority remediation target |
| `0x1C` (decimal `28`) | AES + RC4 | Downgrade-capable — fix by clearing the `0x04` bit |
| `0x18` (decimal `24`) | AES128 + AES256 | Target state — but verify password recency (stale-key trap) |
| `0x00` / missing | Default policy applies | Audit the DC `DefaultDomainSupportedEncTypes`; pre-KB5021131 default still includes RC4 |

**Event-log field (`Ticket Encryption Type` on `4768` / `4769`) — what the KDC *actually issued* on this request:**

| Value | Meaning | Engineering signal |
| --- | --- | --- |
| `0x17` | RC4-HMAC ticket on the wire | Live RC4 in production — migration not done |
| `0x18` | RC4-HMAC-EXP (export-grade RC4) | Same priority as `0x17` — do **not** confuse with the attribute |
| `0x11` | AES128-CTS-HMAC-SHA1-96 ticket | Healthy runtime signal |
| `0x12` | AES256-CTS-HMAC-SHA1-96 ticket | Healthy runtime signal |

**How the two tables work together.** A clean readiness statement requires **both** to be green:

- Directory says `0x18` (no RC4 allowed) **and**
- Recent `4769` events for that SPN show `0x11` / `0x12` (no RC4 actually issued).

Either side green alone is not enough — that is the whole point of §5's outage and the `0x18`-collision trap.

### KDC-side telemetry: Kdcsvc events 201-209 (KB5073381)

The `4768` / `4769` events above tell you *what was issued*, but they don't tell you *what would break if you enforced AES-only today*. That gap is exactly what Microsoft closed with **KB5073381** (January 2026 cumulative updates) — the rollout that retires implicit RC4 fallback by changing the effective default to AES-only when an account's `msDS-SupportedEncryptionTypes` is unset.

The patch introduces **nine new Kdcsvc events (201-209)** in the **System** log (not Security), plus a temporary registry value `RC4DefaultDisablementPhase` that gates the rollout. These events are the **definitive readiness signal** — if your DCs are quiet on 201/202/206/207, you can safely flip to enforcement; if any of them fire, you have an account that *will* break the moment the corresponding 203/204/208/209 starts blocking.

> 🚨 **Don't confuse with KB5021131 (CVE-2022-37966).** That earlier patch (November 2022) closed the *session-key* downgrade — the one referenced throughout §2 and §4. **KB5073381** is a different, later mitigation: it changes the **default value** of `DefaultDomainSupportedEncTypes` to AES-only (`0x18`) for accounts whose `msDS-SupportedEncryptionTypes` is unset. Together they form a chain — KB5021131 (2022) → service-account inventory → KB5073381 (2026) → AES-only default — but they are not interchangeable.

#### The two-stage decision cascade

Before the timeline detail, the **mental model**. Two registry values are at play and they don't override each other — they are read **in cascade** by the KDC whenever an account has no `msDS-SupportedEncryptionTypes` set:

1. **`DefaultDomainSupportedEncTypes`** is the *declarative posture* — what the DC says it offers. If explicitly written into the registry, this value **wins**, regardless of phase.
2. **`RC4DefaultDisablementPhase`** is the *implicit default driver* — it only matters when `DefaultDomainSupportedEncTypes` is **absent**. The phase value determines what the KDC assumes by default.

```text
      KDC receives request for an account WITHOUT
             msDS-SupportedEncryptionTypes
                            │
                            ▼
   ┌─ Step 1 ─────────────────────────────────────────────
   │  DefaultDomainSupportedEncTypes explicitly set?
   │
   │  YES → use that value (always wins)
   │        e.g. 0x18 = AES only, 0x1C = AES + RC4
   │
   │  NO  → fall back to the implicit default ──┐
   └────────────────────────────────────────────┼────────
                                                │
                                                ▼
   ┌─ Step 2 ─────────────────────────────────────────────
   │  Implicit default driven by RC4DefaultDisablementPhase
   │
   │  Phase 0 → default = 0x1C (RC4 allowed)
   │            No 201-209 events
   │
   │  Phase 1 → default = 0x1C (RC4 allowed)
   │            201/202/205/206/207 logged (audit)
   │
   │  Phase 2 → default = 0x18 (AES only)
   │            203/204/208/209 logged (error)
   │            Phase 2 is the default since April 14, 2026
   │
   │  Phase 3 → knob no longer read
   │            default hard-coded to 0x18 in KDC
   │            Effective July 2026
   └──────────────────────────────────────────────────────
```

> 💡 **The practical consequence.** Setting `DefaultDomainSupportedEncTypes = 0x18` explicitly is the only posture that **survives July 2026** — once Phase 3 lands, the phase knob disappears and the only remaining lever to express your intent declaratively is this registry value. Setting `RC4DefaultDisablementPhase` explicitly today is cosmetic (Phase 2 default = `2` already enforces) and becomes a no-op in July.

#### Phased rollout — what actually changes, and when

The rollout looks complicated because it mixes three things that move on different schedules. Separate them once and the rest is easy.

**The knob: a single registry value with three meaningful states.**

`RC4DefaultDisablementPhase` lives at `HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System\Kerberos\Parameters` on every DC and accepts three values:

| Value | Audit events fire? | Implicit RC4 (when account attribute is empty) | Effective `DefaultDomainSupportedEncTypes` fallback |
| --- | --- | --- | --- |
| `0` | No | **Allowed** (legacy behaviour) | Includes RC4 (`0x1C` or whatever was historically there) |
| `1` | **Yes — warnings only** (201, 202, 205, 206, 207) | **Allowed** (audit-only, nothing blocks) | Same as `0` — still includes RC4 |
| `2` | **Yes — errors** (203, 204, 208, 209) | **Blocked** | Forced to `0x18` (AES-only) |

So `0` = silent legacy, `1` = audit, `2` = enforce. **The knob itself doesn't change between January and July 2026**. What changes is the **default value** the KDC assumes when no admin has touched it.

**The clock: three dates, each one moves only the default of that knob.**

```
            BEFORE Jan 13, 2026          ┃ Pre-KB5073381. The knob doesn't exist yet.
                                         ┃ KDC uses legacy default (RC4 in fallback). No 201-209 events.
─────────────────────────────────────────╋──────────────────────────────────────────────────────────────
   ┌───────  Jan 13, 2026 (Phase 1)  ◀───┨ Update installed.
   │     │                               ┃ Knob CREATED. Default value = 0  → still legacy behaviour.
   │     │                               ┃ Admin can opt in to audit by setting it to 1.
   │     │                               ┃ Admin can opt in early to enforcement by setting it to 2.
   │     │                               ┃ Nothing changes for you unless you do something.
   │     ▼
   │   Apr 14, 2026 (Phase 2)  ◀─────────┨ Same update, no new install needed.
   │     │                               ┃ Knob's DEFAULT VALUE flips from 0 to 2 → AES-only by default.
   │     │                               ┃ Errors 203/204/208/209 start firing on every broken request.
   │     │                               ┃ Admin can ROLLBACK by setting the knob back to 0 or 1.
   │     │                               ┃ This rollback is the "manual lifeline" the article warns about.
   │     ▼
   ▼   Jul 2026 (Phase 3)  ◀─────────────┨ Same update, no new install.
                                         ┃ KDC stops reading the knob entirely. Behaviour is locked at 2.
                                         ┃ Setting the registry value to 0 or 1 has no effect anymore.
                                         ┃ Only escape hatch left: explicitly write a non-AES value into
                                         ┃ DefaultDomainSupportedEncTypes (which fires event 205).
```

**The same information as a table, if you prefer:**

| Phase | Date | Default of `RC4DefaultDisablementPhase` | What that produces by default | Manual override possible? |
| --- | --- | --- | --- | --- |
| Pre-patch | before Jan 13, 2026 | n/a (knob doesn't exist) | Legacy behaviour — RC4 implicitly allowed, no events | n/a |
| **Phase 1 — Initial Deployment** | **Jan 13, 2026** | `0` — silent | Legacy behaviour preserved; no audit, no enforcement | Set to `1` for audit, `2` for early enforcement |
| **Phase 2 — Enforcement with manual rollback** | **Apr 14, 2026** | `2` — enforce | AES-only fallback; errors 203/204/208/209 fire on broken requests | Roll back to `0` (silent legacy) or `1` (audit-only) |
| **Phase 3 — Permanent enforcement** | **Jul 2026** | knob no longer read | AES-only fallback locked in code; same errors fire | None via this knob — only via explicit `DefaultDomainSupportedEncTypes` |

**The thing to internalise.**

- The **patch itself** ships once (January). It already contains the events and the enforcement code.
- What ships on April 14 and in July is **not new code** — it's a **default-value change** baked into that same code, gated by the date.
- Admin action is therefore symmetric across the timeline: you can be ahead (set knob to `2` in February) or behind (set knob back to `0` in May) — until July, when the knob is no longer read at all and you lose the rollback option.

> 📌 **Where you actually are right now.** Article 1 was written in May 2026, so by the time you read this you are most likely already past **April 14, 2026** — meaning **your DCs are already enforcing AES-only by default for unspecified accounts**, with errors 203/204/208/209 firing on every request that breaks. If you haven't audited your environment with the warning events first (Phase 1, Jan-Apr), you are now seeing the production version of those breakages. Check the System log on every DC for 203/204/208/209 *before* assuming everything still works.

#### What to do if Phase 2 is breaking your production right now

You have **two levers**, and they are not interchangeable. Reach for the right one based on what you actually know about the breakage.

**Lever A — Roll the knob back to `1` (audit-only) or `0` (silent legacy).** This is the **global panic button**. It restores the legacy fallback (RC4 implicitly allowed) on the DC where you change it, for **every** account that has no explicit `msDS-SupportedEncryptionTypes`. Use it when production is on fire and you don't yet know which accounts are failing — it buys you time to audit properly. Do **not** leave it there: in July 2026 the knob disappears and you'll be back at square one with no warning. If you roll back, you must commit to lever B before July.

**Lever B — Set `msDS-SupportedEncryptionTypes` explicitly on the failing accounts.** This is the **surgical fix**. The errors 203/204/208/209 already tell you exactly which principals are breaking — you scope the exception to those accounts only and the rest of the domain stays AES-only.

| Lever | Scope | Cost | Survives July 2026? | When to use |
| --- | --- | --- | --- | --- |
| **A — knob to `0` or `1`** | Whole DC (all accounts) | One registry change | ❌ No — knob removed | Emergency, you don't know yet *what* is breaking |
| **B — explicit attribute on the account** | One account at a time | One attribute write per principal | ✅ Yes — attribute persists | Once you've identified the legacy principals you can't migrate yet |

**The right reflex is sequential, not "either/or":**

```
Phase 2 errors firing in production
        │
        ▼
[1] Knob → 1   (audit-only — RC4 implicit again, you stop bleeding)
        │
        ▼
[2] Read events 201/202/205/206/207, list affected principals
        │
        ▼
[3] Set msDS-SupportedEncryptionTypes EXPLICITLY on each one
        │   (with AES + RC4 — never RC4-only, see trap below)
        │
        ▼
[4] Knob → 2   (enforce again, exceptions are now declared and traceable)
        │
        ▼
[5] Work the exception list down, one principal at a time, until empty
```

##### The value to put in `msDS-SupportedEncryptionTypes`

When you choose lever B and you **must** keep RC4 working on a specific account, the attribute is a bitmask. The two safe values are:

| Value (decimal / hex) | Meaning | Use when |
| --- | --- | --- |
| `28` / `0x1C` | RC4 + AES128 + AES256 | Standard tolerance — let the KDC pick AES whenever the client can, fall back to RC4 only if it really has to |
| `31` / `0x1F` | RC4 + AES128 + AES256 + DES | Same as above, plus DES — only if you genuinely have a 1990s-era legacy that needs DES |

> ⚠️ **The trap.** Setting `msDS-SupportedEncryptionTypes = 4` (`0x4`, RC4-only) to "make the error go away" is **worse** than rolling the knob back:
>
> - You **forbid** AES on that account permanently — even when the application gets fixed, the KDC won't use AES because the attribute says no.
> - The attribute **survives July 2026**: it's per-account state, not registry state. Your tactical workaround becomes a permanent invisible legacy.
> - You turn temporary debt into permanent debt.
>
> Always include AES in the bitmask alongside RC4. The point of an explicit override is to **add tolerance**, not to **remove options**.

##### Auditing the exception set

Once you start using lever B, you need to be able to list every principal currently carrying an explicit override, so the work to remove them stays visible:

```powershell
# All principals with an explicit msDS-SupportedEncryptionTypes (any value)
Get-ADObject -LDAPFilter '(msDS-SupportedEncryptionTypes=*)' `
    -Properties msDS-SupportedEncryptionTypes, ObjectClass, whenChanged |
    Select-Object Name, ObjectClass, msDS-SupportedEncryptionTypes, whenChanged |
    Sort-Object msDS-SupportedEncryptionTypes, Name

# Just the ones still allowing RC4 (bit 0x4 set)
Get-ADObject -LDAPFilter '(msDS-SupportedEncryptionTypes=*)' `
...
The second query is the one that matters for July 2026 readiness: anything still in that list is an exception you must close before the knob disappears.

#### The 9 events, decoded

Two AD knobs drive everything in this sub-section, and you need both in mind to read the events:

- **`msDS-SupportedEncryptionTypes`** — a **per-account** AD attribute (user, computer, gMSA, TDO). It declares *"these are the etypes the KDC is allowed to use for this specific principal"*.
- **`DefaultDomainSupportedEncTypes`** — a **per-DC** registry value (`HKLM\SYSTEM\CurrentControlSet\Services\Kdc`). It's the **fallback** the KDC applies when `msDS-SupportedEncryptionTypes` on the account is empty or `0` — whichever DC handles the request uses its own local registry value.

Together those two values are the **policy** — what AD **declares is allowed**. Next to the policy lives the **physical reality** of which AES keys actually exist on disk in `supplementalCredentials`, which can be missing even when policy permits AES (the §2 🔑 trap).

##### The mental grid: 2 questions, 4 cells

When the KDC processes a Kerberos request that ends up needing RC4, it's effectively asking two questions:

1. **What does policy say?** Is the service's `msDS-SupportedEncryptionTypes` empty (KDC falls back to the DC's `DefaultDomainSupportedEncTypes`), or explicitly set to `0x18` (AES-only)?
2. **What's actually broken in this request?** Is it the **client** that can't speak AES? Or is it the **service** that has no AES key on disk (stale-key trap)?

Cross those two questions and you get the four cells Microsoft instrumented:
...
```
                    ...
                    │  is empty / 0                    │  = 0x18 (AES-only)               │
                    │  → KDC falls back to             │                                  │
                    │    DefaultDomainSupportedEncTypes│                                  │
   ─────────────────┼──────────────────────────────────┼──────────────────────────────────┤
   Client RC4-only  │   201  →  203                    │   206  →  208                    │
   (CLIENT can't    │   warning   error                │   warning   error                │
    speak AES)      │                                  │                                  │
   ─────────────────┼──────────────────────────────────┼──────────────────────────────────┤
   Service has no   │   202  →  204                    │   207  →  209                    │
   AES key on disk  │   warning   error                │   warning   error                │
   (stale-key trap) │                                  │                                  │
   ─────────────────┴──────────────────────────────────┴──────────────────────────────────┘

   Off-grid: a single hygiene-only event
   205 → an admin has manually re-enabled RC4 (or DES) in this DC's
         DefaultDomainSupportedEncTypes registry value. Never blocking — just flags
         the DC as less secure than the rest of the fleet. Logged once per Kdcsvc start.
```

Each cell holds a **warning → error pair**: the warning is what you see in Phase 1 (audit), the error is what you'll see in Phase 2 (enforcement) **for the same request, on the same client/service pair**. Understand one cell, you've understood four events.

##### Left column: the service's `msDS-SupportedEncryptionTypes` is empty (the most common case)

This is the inherited state of 90 % of legacy accounts: nobody ever populated the attribute, so the KDC falls back to the DC's `DefaultDomainSupportedEncTypes`. Before April 2026 that fallback included RC4 → everything worked silently. Since April 2026 the effective fallback is `0x18` → it breaks, and these are exactly the events that surface those breakages.

| Phase 1 (warning) | Phase 2 (error) | Trigger | What it actually means |
| --- | --- | --- | --- |
| **201** | **203** | The **client** advertises only RC4 in its `etype list`, the service has no `msDS-SupportedEncryptionTypes` set, and the DC has no explicit `DefaultDomainSupportedEncTypes` | The **client** is the one stuck on RC4. Either its OS is too old for AES, or its `krb5.conf` / keytab pins it to RC4 (typical case: Java / Heimdal / MIT appliance). **Action**: fix the client. If you can't, set `msDS-SupportedEncryptionTypes = 0x24` (= RC4 + AES session keys) on the **service** as a temporary lifeline — it re-allows RC4 for that one service only, without touching the rest of the domain. |
| **202** | **204** | The **service** has no AES keys in `supplementalCredentials`, no `msDS-SupportedEncryptionTypes` set, and no explicit `DefaultDomainSupportedEncTypes` | **The stale-key trap firing in production** (the §6 🪤 callout). The service account password predates 2008 or hasn't been rotated since the RC4-only era — so the KDC never generated AES keys for it. **Action**: rotate the service account password. That single action runs the password through the §2 🔑 KDF pipeline and creates the missing AES keys. No need to touch the attribute. |

##### Right column: the service's `msDS-SupportedEncryptionTypes = 0x18` (or DC's `DefaultDomainSupportedEncTypes = 0x18`)

Same failure structure as the left column, but this time the **policy is explicit**: someone wrote `0x18` into the attribute, so AD says *"this service does AES only, period"*. The KDC doesn't even need a fallback — it refuses outright if AES isn't possible. The next two events are the mirror of 201/202 for that strict policy.

| Phase 1 (warning) | Phase 2 (error) | Trigger | What it actually means |
| --- | --- | --- | --- |
| **206** | **208** | The **client** advertises only RC4 *and* the service has `msDS-SupportedEncryptionTypes = 0x18` (or the DC has `DefaultDomainSupportedEncTypes = 0x18`) | Same symptoms as 201/203, but this time AES-only is **explicitly assumed**. From a triage standpoint that's reassuring: someone made the effort to set `0x18`, so the attribute is correct; the problem is purely client-side. **Action**: same — fix the client, or temporary `0x24` lifeline on the service. |
| **207** | **209** | The **service** has no AES keys *and* `msDS-SupportedEncryptionTypes = 0x18` (or the DC has `DefaultDomainSupportedEncTypes = 0x18`) | **The 🪤 trap in its most painful form**: the attribute declares *"I do AES only"*, but disk reality says *"I have zero AES keys"*. Policy is lying to itself. The KDC has no option and refuses. **Action**: rotate the password immediately — this aligns reality (keys on disk) with declaration (`0x18`). This is exactly the scenario the §6 reminder is meant to prevent, playing out for real. |

##### Off-grid: a single hygiene event (never blocking)

| Event | When it fires | What it tells you |
| --- | --- | --- |
| **205** | A DC has `DefaultDomainSupportedEncTypes` **explicitly set** to a value containing anything other than AES-SHA1 (i.e. RC4 or DES bits set). Logged **once at every Kdcsvc startup**. | An admin has deliberately put RC4 (or DES) back into that DC's `DefaultDomainSupportedEncTypes`. Microsoft will **never block** this config — explicit admin intent always overrides Microsoft defaults. But the event fires to flag the DC as less secure than the rest of the fleet. **Action**: ask whether the override is still justified. If not, clear the value — the new `0x18` default takes back over automatically. |

##### One-line recap of the grid

- Event ID is **even** (202, 204, 207, 209) → **service-side** problem (missing AES keys, stale-key trap) → fix = **rotate password**.
- Event ID is **odd** within 201/203/206/208 → **client-side** problem → fix = **fix the client** (or `0x24` lifeline on the service).
- Event is **205** → **DC-side** problem (`DefaultDomainSupportedEncTypes` registry override) → fix = **review the override**.
- 201/202 vs 206/207 = same failure, **implicit vs explicit policy**.
- Phase 1 (warning) vs Phase 2 (error) = same failure, **before vs after April 14, 2026**.

#### How these fit into the readiness workflow

| You see... | Phase | Means | Do |
| --- | --- | --- | --- |
| **Nothing — silent System log** | 1 (Jan-Apr 2026) | Either the registry is still at `0` (no auditing) or no RC4 dependencies remain | Set `RC4DefaultDisablementPhase = 1` to actively enable warning events; if still silent for a representative cycle (weekends, batch, partner traffic), you're safe to flip to `2` |
| **201 / 202 / 206 / 207 warnings** | 1 | Live RC4 dependency identified, but not yet blocking. **You have a window to fix.** | Triage by event: 201/206 = client side, 202/207 = stale key. Fix all of them before April 2026 (or before manually setting registry to `2`). |
| **203 / 204 / 208 / 209 errors** | 2 (post-April 2026) | **Authentication is failing right now** for that account/SPN | Either fix immediately, or temporarily set `msDS-SupportedEncryptionTypes = 0x24` on the service to unblock while you remediate. **Do not** roll the registry back to `1` for the whole domain to mask one bad account — that re-exposes everything KB5073381 was meant to close. |
| **205 warning at KDCSVC startup** | 1 or 2 | A DC has `DefaultDomainSupportedEncTypes` explicitly set to an insecure value (RC4 or DES bits) | Audit the registry on that DC and align with the rest of the fleet |

> 🧠 **Why these events matter more than 4769 for readiness validation.** A `4769` with `Ticket Encryption Type = 0x17` tells you RC4 was issued — but it doesn't tell you *whether AES would have worked instead*. Events 201/202/206/207 do exactly that: they are the KDC saying *"I am about to (or just did) issue RC4, but in enforcement mode I would have failed this request — here is the account, here is the client."* That is the missing decision-grade signal: pre-enforcement evidence of what will break, **per request, with full account context**. Article 2 turns this into concrete System log queries alongside the 4768/4769 baselines.

### When evidence is incomplete: failure-mode → first action

In real triage you almost never have the full evidence set in front of you — you have **one signal** (a 4769 line, a Kdcsvc event, a broken app) and need a first move. The table below is ordered by **how much evidence you actually hold**, from least to most: *"I changed config and see nothing"* → *"I have ambiguous runtime data"* → *"I have a precise Kdcsvc warning"* → *"I have a Kdcsvc error and prod is down"* → *"the failure crosses a trust boundary"*.

Same single rule under all of it: **never roll back broader than the failure**. A specific account breaks → fix that account, not the domain default.

| You have… | Symptom | Likely cause | First technical action |
| --- | --- | --- | --- |
| **No runtime evidence yet** | AES policy applied (GPO / attribute) but `4768`/`4769` telemetry unchanged | Static config changed without runtime validation — the §5 outage scenario | Build the 4768/4769 baseline first (article 2), and enable Kdcsvc warnings (`RC4DefaultDisablementPhase = 1`) before any enforcement |
| **Ambiguous runtime evidence** | `4769` still shows RC4 (`Ticket Encryption Type = 0x17`) for the same SPNs after "hardening" | Two roots possible — (a) `msDS-SupportedEncryptionTypes` still has the `0x04` bit, or (b) bitmask is `0x18` but no AES key on disk (stale-key trap) | Disambiguate with Kdcsvc events: 201/206 → root (a), client; 202/207 → root (b), stale key. Then act on whichever fired |
| **Precise Kdcsvc warning — service side** | Kdcsvc **202 / 207** firing on a service account (Phase 1, pre-April 2026 or `RC4DefaultDisablementPhase = 1`) | **Stale-key trap** — bitmask declares AES but `supplementalCredentials` has no AES key | Rotate the service account password immediately — that single action runs the §2 🔑 KDF pipeline and creates the missing AES keys. Re-check System log on next request |
| **Precise Kdcsvc warning — client side** | Kdcsvc **201 / 206** firing from a specific source IP / hostname | Client-side enctype lock (`krb5.conf`, keytab, vendor appliance pinning RC4) | Fix the client (preferred). If you can't, set `msDS-SupportedEncryptionTypes = 0x24` on **the service** as a temporary lifeline — never re-enable RC4 domain-wide |
| **Kdcsvc error — production is broken** | Kdcsvc **203 / 204 / 208 / 209** firing after April 14, 2026 | Phase-2 enforcement is blocking auth — same root causes as the warnings, but the KDC is now refusing the request | Triage the failing account: 203/208 = fix client (or `0x24` lifeline); 204/209 = rotate password. **Do not** set `RC4DefaultDisablementPhase = 1` for the whole domain to mask one bad account — that re-exposes everything KB5073381 was meant to close |
| **Cross-trust failure** | Cross-forest / cross-domain auth breaks after enforcement; on-domain auth still works | TDO posture incomplete — either `msDS-SupportedEncryptionTypes` on the trust object is wrong, or the trust password hasn't been rotated since the RC4-only era (stale-key trap on a TDO) | Audit TDO settings **and** rotate the trust password — see [trust hardening annex](../Hardening%20Kerberos%20Encryption%20on%20AD%20Trusts/Hardening%20Kerberos%20Encryption%20on%20AD%20Trusts.md) |

> 🪤 **205 is absent from this table on purpose.** It's a hygiene-only event (a DC has `DefaultDomainSupportedEncTypes` explicitly set to an insecure value) — it never blocks auth and never escalates to an error, so it doesn't belong in a *"something is breaking, what do I do first"* triage. Handle it as a periodic fleet-consistency check, not as an incident signal.

### Scope: what this article covers and what it doesn't

§6 covers the **readiness signals** end-to-end: directory posture, KDC default posture, the bitmask decoder, the 4768/4769 runtime evidence, and the new Kdcsvc 201-209 KDC-side telemetry from KB5073381. The actual **rollout sequencing** — `RC4DefaultDisablementPhase` value transitions per DC, `KrbtgtFullPacSignature` interaction with CVE-2022-37967, structured exception handling for non-Windows keytabs (Java, Heimdal, MIT) with the `0x24` workaround, and the post-April-2026 / pre-July-2026 rollback window decisions — belongs to **article 3** and the dedicated annexes. The split is deliberate: readiness criteria are generic and stable, but rollout sequencing depends on the Microsoft patch cycle and on which non-Windows clients you have to keep alive during the transition.

## 7. From baseline to action: what to do next

This article gave you the decoder — what RC4 is, why it's dangerous, where it sits in the Kerberos flow, what evidence proves your estate is ready for AES-only, and which pitfalls (stale-key trap, `0x18` namespace collision, mis-attributed post-mortems) most often derail the migration. Articles 2 and 3 turn that into queries and a rollout. Before moving on, there are four concrete actions worth doing in the meantime — they cost almost nothing, and they're the difference between *"we'll start the inventory next quarter"* and *"we already have the data when we open Article 2"*.

### Minimum viable readiness check — do this first

A 4-step sequence, anchored to the Kdcsvc evidence model from §6. Every step has a clear stop condition.

1. **Confirm patch level on every DC.** KB5073381 (January 2026 cumulative) is the prerequisite for the 201-209 events to exist at all. Verify with `Get-HotFix` or build number across the whole DC fleet — partial patching means partial telemetry, which is worse than none (you'll think you have evidence and you'll be missing events from the un-patched DCs).
2. **Read each DC's `RC4DefaultDisablementPhase`** at `HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System\Kerberos\Parameters`. If unset, the effective value is `0` (silent — no warnings logged). Set to `1` on every DC to start collecting Phase-1 warnings *without* changing enforcement behavior. **As of June 2026 this is purely additive** — your DCs are already enforcing AES-only by default since April 14, 2026; flipping the registry to `1` only opens the audit channel, it doesn't tighten anything.
3. **Wait one full business cycle** before reading the System log. Minimum 7 days; ideally a month-end window so you cover monthly batch jobs, weekend backups, partner connections, and any quarterly process. Anything shorter risks missing the legacy flow that runs once per month and breaks when you finally enforce.
4. **Triage what you find.** Use the §6 grid:
   - **Nothing in System log** → either no remaining RC4 dependency (good) or your audit window was too short (re-run). Cross-check with `4769` events showing `Ticket Encryption Type = 0x17` to disambiguate.
   - **201/206 warnings** → client-side enctype lock. Fix the client.
   - **202/207 warnings** → stale-key trap. Rotate the service password.
   - **205 warnings** → DC `DefaultDomainSupportedEncTypes` registry override. Audit and align.
   - **203/204/208/209 errors** → production is already broken on Phase-2 enforcement. Fix per-account, never roll the registry back to `1` for the whole domain to mask one bad account — that re-exposes everything KB5073381 was meant to close.

If you complete steps 1-4 cleanly with no remaining warnings/errors, your readiness baseline is essentially done — Article 2 will only widen the angle (4768/4769 baselines, trust audit, exception classification) on top of an already-solid foundation.

### The rest of the series picks it up here

- **[Article 2 — Legacy Dependency Mapping and Technical Inventory](./2.%20Legacy%20Dependency%20Mapping%20and%20Technical%20Inventory.md)** translates this readiness model into concrete queries: the 4768/4769 field map (with the `0x18` namespace trap clearly separated from the attribute namespace), the Kdcsvc 201-209 System-log query baseline, the trust-object inventory pattern (TDO bitmask + password age cross-check), and the residual-RC4 classification template (*remediable now* vs *controlled exception*) with owner and target date.
- **[Article 3 — Remediation, AES Migration, Cutover, and RC4 Extinction](./3.%20Remediation%2C%20AES%20Migration%2C%20Cutover%2C%20and%20RC4%20Extinction.md)** turns the inventory into a rollout: per-DC `RC4DefaultDisablementPhase` transition sequence, structured handling of non-Windows keytabs (Java / Heimdal / MIT) with the `0x24` workaround, the `KrbtgtFullPacSignature` interaction with CVE-2022-37967, the post-April-2026 / pre-July-2026 rollback window calculus, and the discipline around AES-but-long-password (the M1027 backstop from §4) — without which "AES-only" is only a speed bump, not a permanent fix.

### Related technical references in this repository

These are deeper dives or operational runbooks that complement the series — read them on demand, not in sequence:

- **[`Kerberos Encryption Audit/Invoke-KerberosEncryptionAudit.ps1`](./Kerberos%20Encryption%20Audit/Invoke-KerberosEncryptionAudit.ps1)** — the read-only audit script that automates the inventory phase: KDC default per DC, account `msDS-SupportedEncryptionTypes` posture, and 4768/4769 traffic correlated into an HTML / JSON / CSV report. Documented in Article 2 §6 and in the dedicated [`How to use Invoke-KerberosEncryptionAudit Script.md`](./Kerberos%20Encryption%20Audit/How%20to%20use%20Invoke-KerberosEncryptionAudit%20Script.md).
- **[Hardening Kerberos Encryption on AD Trusts](../Hardening%20Kerberos%20Encryption%20on%20AD%20Trusts/Hardening%20Kerberos%20Encryption%20on%20AD%20Trusts.md)** — the trust hardening annex referenced throughout this article. Goes deep on TDOs, the stale-key trap on cross-forest trusts, and the password-rotation procedure that actually generates the AES keys.
- **[RC4 HMAC Report (Defender for Identity)](../../../020%20-%20Microsoft%20Defender/Defender%20for%20Identity/Reporting/RC4%20HMAC%20Report.md)** — how to read and interpret the MDI built-in RC4 report, including the *"RC4 session key"* signal and how it maps (or doesn't) to the §4 attackable-envelopes table.
