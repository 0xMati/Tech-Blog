...
The events come in **two pairs of two pairs** — warning (Phase 1) → error (Phase 2), with a separate hygiene event (`205`) that never escalates:

**Account has no `msDS-SupportedEncryptionTypes` defined (relies on KDC default):**

| Phase 1 (warning) | Phase 2 (error) | Trigger condition | What it tells you |
| --- | --- | --- | --- |
| **201** | **203** | Client only advertises RC4 *and* target service has no `msds-SET` *and* DC has no explicit `DDSET` | Client side is RC4-locked. Either the client can't do AES or its keytab has only RC4 keys. **Action**: fix the client, or set `msDS-SupportedEncryptionTypes = 0x24` (RC4 + AES session keys) on the service as a temporary keytab workaround. |
| **202** | **204** | Service account has **no AES keys in `supplementalCredentials`** *and* no `msds-SET` *and* DC has no explicit `DDSET` | **The stale-key trap firing in production.** The service account password predates AES support or has never been rotated since RC4-only days. **Action**: rotate the service account password — that single action regenerates AES keys via the KDF pipeline (§2 🔑). |

**Account has `msDS-SupportedEncryptionTypes` explicitly set to AES-SHA1 only (or DC has `DDSET = 0x18`):**

| Phase 1 (warning) | Phase 2 (error) | Trigger condition | What it tells you |
| --- | --- | --- | --- |
| **206** | **208** | Client only advertises RC4 *and* target service has `msds-SET = 0x18` (or DDSET = `0x18`) | Same as 201/203 but the policy is **explicit**, not implicit. Client-side enctype lock against an AES-only service. **Action**: same — fix the client or temporarily allow RC4 via `0x24`. |
| **207** | **209** | Service account has no AES keys *and* `msds-SET = 0x18` (or DDSET = `0x18`) | **Stale-key trap on a service that's been declared AES-only.** Bitmask says AES, no AES key on disk, request fails. **Action**: rotate the service password immediately. This is the §6 reminder callout firing for real. |

**Hygiene-only event (never escalates to error):**

| Event | Trigger | What it tells you |
| --- | --- | --- |
| **205** | DC has `DefaultDomainSupportedEncTypes` **explicitly set** to a value containing anything other than AES-SHA1 (i.e. RC4 or DES bits set). Logged once at every Kdcsvc start. | An admin has manually re-enabled RC4 at the KDC default level. Microsoft will **never block** this — it respects explicit admin intent — but it does flag the DC as insecure. **Action**: review whether that override is still needed. If not, clear it; the new default (`0x18`) is what you want. |
