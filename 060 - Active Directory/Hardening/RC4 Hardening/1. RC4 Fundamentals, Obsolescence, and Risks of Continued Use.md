...
Kerberos in everyday language:

- Kerberos = a movie theater entry system.
- The KDC (Key Distribution Center) = the box office (a domain controller).
- A TGT (Ticket Granting Ticket) = your VIP pass: it does not let you into a movie, but it lets you ask for specific movie tickets.
- A service ticket = the actual movie ticket for one specific room (one specific service, like SQL or a file share).
- A session key = a secret handshake shared between two parties for one visit only.
- A long-term key = a stable badge linked to an account password (user, computer, service, or `krbtgt`).

Why RC4 matters in this story:

- Tickets and handshakes can be "sealed" with AES or with RC4.
...
If the previous box-office analogy is clear, this section will feel natural.
...
- You arrive at the box office (KDC).
- You prove you are you by showing something only you know (your password, indirectly).
- The box office gives you a VIP pass (TGT) plus a one-day secret handshake (session key) so you can come back later and ask for specific tickets.
...
- You go back to the box office, show your VIP pass (TGT), and ask for a ticket to a specific movie room (a service like SQL).
- The box office prints a ticket that only that room can read, and gives you a new secret handshake (session key) for that room.
- You walk to the room, hand over the ticket and use the new handshake to prove you are the right person.
