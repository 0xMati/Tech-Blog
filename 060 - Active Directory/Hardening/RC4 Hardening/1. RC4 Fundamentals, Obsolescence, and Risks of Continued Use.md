...
>
> These two can disagree — and this is the heart of the November 2022 KDC change (KB5021131): the session key now defaults to AES even when the envelope is RC4, which closes the largest Kerberoasting blind spot in legacy estates. We will come back to this distinction in §1's deep dives, in the audit field map (§1 *4768/4769*), and again when discussing trust hardening.

### Notation used in the rest of this article

A Kerberos ticket is a **sealed envelope**. The seal is always made with a long-term key — written **`K_krbtgt`** when it is the krbtgt account's key (TGT), and **`K_svc`** when it is the service account's key (service ticket / TGS). Inside the envelope sits a session key plus identity information. The Deep Dives below use this notation throughout.
