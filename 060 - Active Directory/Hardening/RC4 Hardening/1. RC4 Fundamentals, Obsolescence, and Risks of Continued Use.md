...
#### Phased rollout — where you sit on the timeline matters

| Phase | Date | What changes | `RC4DefaultDisablementPhase` value | Events fired |
| --- | --- | --- | --- | --- |
| **Initial Deployment** | **January 13, 2026** | Update introduces the new events and the registry key. Default behavior **unchanged** — RC4 still implicitly allowed when attribute is `0`. | `0` (default) — no audit, no change. Set to `1` to enable warning events. | Warnings: 201, 202, 205, 206, 207 (when registry = `1`) |
| **Enforcement with manual rollback** | **April 14, 2026** | Default value of `DefaultDomainSupportedEncTypes` flips to **AES-SHA1 only (`0x18`)** for accounts with no explicit attribute. Enforcement is **automatic** but can be rolled back by setting registry to `0` or `1`. | Effectively `2` unless overridden. | Errors: 203, 204, 208, 209 (replace the matching warnings) — plus 205 still fires if `DefaultDomainSupportedEncTypes` is explicitly insecure |
| **Permanent enforcement** | **July 2026 onward** | Registry key `RC4DefaultDisablementPhase` is **removed**. No rollback path. AES-only default is permanent unless `DefaultDomainSupportedEncTypes` is explicitly set otherwise. | Key no longer read. | Errors only — 203, 204, 208, 209 |

> 📌 **Where you actually are right now.** Article 1 was written in May 2026, so by the time you read this you are most likely already past **April 14, 2026** — meaning **your DCs are already enforcing AES-only by default for unspecified accounts**. If you haven't audited your environment with these events first, you may already be experiencing silent breakage. Check the System log on every DC for events 203/204/208/209 *before* assuming everything still works.
