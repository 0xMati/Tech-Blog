...
### Beginner deep dive: where the session key lives and who can decrypt what

If you are new to Kerberos, keep this mental model:

- The client carries tickets, but usually cannot read ticket internals.
- The KDC generates temporary session keys.
- The same session key is provided to both parties, but through different encrypted containers.
...
#### AS flow (getting a TGT)

1. Client sends AS-REQ to KDC.
2. KDC creates a client-KDC session key.
3. KDC sends AS-REP containing:
	- TGT (with `EncTicketPart` encrypted using `krbtgt` long-term key).
	- Reply `enc-part` for client (contains same session key for client use).

Outcome:

- Client can read its reply `enc-part` and gets session key.
- Client cannot decrypt TGT internal blob.
- KDC can later decrypt TGT internals via `krbtgt` key.

#### TGS flow (getting a service ticket)

1. Client sends TGS-REQ with:
	- TGT.
	- Authenticator proving possession of client-KDC session key.
	- Requested SPN/service.
2. KDC validates and creates a client-service session key.
3. KDC sends TGS-REP containing:
	- Service ticket (`EncTicketPart` encrypted with service long-term key).
	- Reply `enc-part` to client (contains same client-service session key).

Outcome:

- Client learns client-service session key.
- Client still cannot decrypt service ticket internals.
- Service can decrypt ticket internals with its long-term key and recover same session key.

#### AP flow (using the service ticket)

1. Client sends AP-REQ to service:
	- Service ticket.
	- Authenticator encrypted with client-service session key.
2. Service decrypts ticket, extracts session key, verifies authenticator.
...
AS/TGT decryption map:

| Step | Artifact | Encrypted with | Who can decrypt | Contains |
| --- | --- | --- | --- | --- |
| 1 | AS-REQ pre-auth proof | Derived from `K_user` context | KDC | Proof client knows account secret |
| 2 | TGT `EncTicketPart` | `K_krbtgt` | KDC (`krbtgt`) only | Client identity, ticket flags, lifetime, `K_c,tgs` |
| 3 | AS-REP `enc-part` | Client-readable key context (`K_user`) | Client | Copy of `K_c,tgs`, validity data |
...
TGS/service ticket decryption map:

| Step | Artifact | Encrypted with | Who can decrypt | Contains |
| --- | --- | --- | --- | --- |
| 1 | TGS-REQ authenticator | `K_c,tgs` | KDC | Fresh proof client holds TGT session key |
| 2 | Service ticket `EncTicketPart` | `K_svc` | Target service only | Client identity, authorization data, `K_c,s` |
| 3 | TGS-REP `enc-part` | `K_c,tgs` | Client | Copy of `K_c,s`, ticket metadata |
| 4 | AP-REQ authenticator | `K_c,s` | Target service | Proof client holds same session key as ticket |
...
#### Where RC4/AES confusion is born in real investigations
