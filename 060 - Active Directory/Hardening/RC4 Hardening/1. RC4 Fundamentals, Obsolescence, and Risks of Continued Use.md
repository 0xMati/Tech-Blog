...
- **KDC default posture** — `DefaultDomainSupportedEncTypes` registry value on each DC (`HKLM\SYSTEM\CurrentControlSet\Services\Kdc`). This is the **fallback** the KDC applies when an account's `msDS-SupportedEncryptionTypes` is `0` or missing — i.e. when the directory says nothing. Not replicated; it's a per-DC registry value, so it has to be checked (and aligned) on every DC. Two distinct Microsoft changes touch this layer and are commonly conflated — keep them separate:
    ...
    - **April 2026 (KB5073381 / CVE-2026-20833)** changed the **ticket envelope** default: when `msDS-SupportedEncryptionTypes` is unset, the effective `DefaultDomainSupportedEncTypes` flips to **AES-only (`0x18`)**. This is the change that finally retires implicit RC4. Detailed in the Kdcsvc 201-209 sub-section below.
...
### KDC-side telemetry: Kdcsvc events 201-209 (CVE-2026-20833 / KB5073381)

The `4768` / `4769` events above tell you *what was issued*, but they don't tell you *what would break if you enforced AES-only today*. That gap is exactly what Microsoft closed with **KB5073381** (January 2026 cumulative updates) for **CVE-2026-20833** — an information-disclosure vulnerability where a KDC could be coerced into issuing RC4 service tickets even when AES was available.
...
`RC4DefaultDisablementPhase` lives at `HKLM\SYSTEM\CurrentControlSet\Services\Kdc\Parameters` on every DC and accepts three values:
...
| **203 / 204 / 208 / 209 errors** | 2 (post-April 2026) | **Authentication is failing right now** for that account/SPN | Either fix immediately, or temporarily set `msDS-SupportedEncryptionTypes = 0x24` on the service to unblock while you remediate. **Do not** roll the registry back to `1` for the whole domain to mask one bad account — that re-exposes everything to CVE-2026-20833. |
...
4. **Triage what you find.** Use the §6 grid:
   ...
   - **203/204/208/209 errors** → production is already broken on Phase-2 enforcement. Fix per-account, never roll the registry back to `1` for the whole domain to mask one bad account — that re-exposes everything to CVE-2026-20833.
