...
> ℹ️ **Default `0x1C` on a healthy Windows computer account is *not* a finding.** Per §0, the Microsoft baseline value of `0x1C` is acceptable on Windows computers with active machine-password rotation: the password is a random ~120-character secret rotated every 30 days, RC4 cracking against it is unrealistic, and the KDC excludes RC4 from negotiation as soon as AES is honourable. Tighten to `0x18` only when the auto-rotation is broken, paused, or the host is a non-Windows appliance with a static or weak machine secret.
...
4. **Tighten capability only if rotation is not healthy.** For a standard Windows computer account with active 30-day machine-password rotation, **leave `msDS-SupportedEncryptionTypes = 0x1C` unchanged** — that is the Microsoft baseline (cf. §0). Tighten to `0x18` only on appliances or hosts where the auto-rotation is disabled, paused, or unreliable, and only after step 3 has confirmed AES keys are present on disk.
...
- Permanent control: quarterly review of service principals (user accounts holding an SPN, gMSA, sMSA) whose `msDS-SupportedEncryptionTypes` is not `0x18` — every such principal must map to an active exception. Computer accounts at default `0x1C` are excluded from this review (Microsoft baseline, see §3.2).
...
| **Principal type** | Service account (user-with-SPN). For computer accounts at default `0x1C`, see §3.2 — those are baseline, not exceptions. |
...
| **Service principals** (user-with-SPN, gMSA, sMSA) with `msDS-SupportedEncryptionTypes` allowing RC4 (bit `0x04`) | Equal to the §7 exception list count — no surprises. *Computer accounts at default `0x1C` are excluded (Microsoft baseline, see §3.2).* | AD attribute scan |
...
- **Account onboarding policy** — any new **service principal** (user account holding an SPN, gMSA, sMSA) or **trust object** must be created with `msDS-SupportedEncryptionTypes = 0x18` from day one. Bake it into your provisioning tooling. Computer accounts created via standard domain-join remain at the Microsoft default `0x1C` (see §3.2) — that is by design.
