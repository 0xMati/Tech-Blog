...
- (Optional) Microsoft 365 service tag CIDRs for region-specific filtering.

**Steps**
1. **Open Traffic Profiles**  
   - Navigate to **Global Secure Access > Traffic profiles** in the Entra admin center.
2. **Create Profile**  
   - Click **+ New profile** → select **Microsoft**.  
   - Name: ``MS365-Profile``.
3. **Define Filters**  
   - **Include Domains**:  
     - ``*.microsoftonline.com``  
     ...
     - ``*.teams.microsoft.com``  
     - ``*.graph.microsoft.com``  
   - (Optional) **Include IP ranges**: paste service tag CIDRs.
4. **Configure Exclusions**  
   - **Exclude IP ranges**: internal subnets (e.g., ``10.1.0.0/16``) to avoid double-inspection.
5. **Save & Apply**  
   - Click **Save**.  
   - Go to **Global Secure Access > Connect > Traffic forwarding**.  
   - Toggle on ``MS365-Profile`` for your pilot users or remote network.  
   - Wait 5–10 minutes for propagation.

> 💡 **Pro Tip:** Use wildcard domains to automatically cover newly added endpoints.
