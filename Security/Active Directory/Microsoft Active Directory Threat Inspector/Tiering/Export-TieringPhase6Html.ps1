...
# Generates a rich HTML report for Phase 6 — GPO Hardening Per Tier.
...
function Export-TieringPhase6Html {
    ...
    $blockInh = $Results.BlockInheritance
    ...
    foreach ($s in $Results.SettingsApplied) {
        $b = if ($s.Status -eq 'Applied') { "<span class='badge pass'>Applied</span>" } else { "<span class='badge warn'>$($s.Status)</span>" }
        $settingsRows += "<tr><td>$(HtmlEncode $s.GPO)</td><td>$(HtmlEncode $s.Setting)</td><td>$b</td></tr>`n"
    }
...
.info-banner{background:var(--card);border:1px solid var(--border);border-left:4px solid var(--accent);border-radius:8px;padding:1rem 1.5rem;margin-bottom:1.5rem;font-size:.9rem}
...
<p class="subtitle">GPO Hardening Per Tier | Domain: $(HtmlEncode $DomainDN) | Generated: $timestamp</p>
...
$(if($blockInh){"<div class='info-banner'><strong>&#x1F512; Block Inheritance</strong> enabled on Tier 0 OU. Domain-level GPOs no longer flow down to Tier 0.</div>"})
</div>
...
<div class="step-card"><div class="step-number">1</div><div class="step-text"><strong>Verify with gpresult</strong> — Run <code>gpresult /r</code> on pilot machines per tier to confirm GPO application.</div></div>
<div class="step-card"><div class="step-number">2</div><div class="step-text"><strong>Configure LAPS permissions</strong> — Grant read access: T0-Admins for T0, T1-Admins for T1, T2-Admins for T2.</div></div>
<div class="step-card"><div class="step-number">3</div><div class="step-text"><strong>Audit NTLM usage</strong> — Enable NTLM audit on DCs for 30+ days before restricting.</div></div>
<div class="step-card"><div class="step-number">4</div><div class="step-text"><strong>Phase 7 — Tier 0 Object Protection</strong> — Audit ACLs, secure GPOs, rotate krbtgt, harden service accounts.</div></div>
</div></div>

<p class="note" style="text-align:center;margin-top:2rem;">Generated by MATI — Phase 6 — GPO Hardening Per Tier</p>
</div></body></html>
"@
