...
# Generates a rich HTML report for Phase 5 — Create PAW Hardening GPOs.
...
function Export-TieringPhase5Html {
    ...
    foreach ($target in $Results.Targets) {
        $targetRows += "<tr><td>$(HtmlEncode $target.Tier)</td><td>$(HtmlEncode $target.Label)</td><td class='mono'>$(HtmlEncode $target.TargetOU)</td></tr>`n"
    }
...
<p class="subtitle">Create PAW Hardening GPOs | Domain: $(HtmlEncode $DomainDN) | Generated: $timestamp</p>
...
<div class="advisory"><strong>Microsoft PAW recommendation:</strong> Microsoft recommends adding WDAC / App Control for Business to privileged access workstations, starting in audit mode and moving to enforcement only after the authorized software set is validated. This Phase 5 report covers firewall, Credential Guard, HVCI, BitLocker, and loopback, but not WDAC deployment itself.</div></div>

<div id="targets"><h2 class="section-header"><span class="section-icon">&#x1F3AF;</span> Targets</h2>
<div class="card"><table><thead><tr><th>Tier</th><th>Scope</th><th>Target OU</th></tr></thead><tbody>$targetRows</tbody></table></div></div>
...
<div class="step-card"><div class="step-number">1</div><div class="step-text"><strong>Finalize tier allow lists</strong> — Review <code>Config\Tiering.config.psd1</code> and populate Tier0/Tier1/Tier2 allowed remote addresses for any endpoints not auto-discovered.</div></div>
<div class="step-card"><div class="step-number">2</div><div class="step-text"><strong>Review generated firewall GPOs</strong> — Open GPMC and validate Domain, Private, and Public profiles plus outbound allow/block rules for each tier before broad deployment.</div></div>
<div class="step-card"><div class="step-number">3</div><div class="step-text"><strong>Deploy PAW hardware</strong> — Physical machines with TPM 2.0, Secure Boot, clean OS install, and driver set validated for HVCI compatibility.</div></div>
<div class="step-card"><div class="step-number">4</div><div class="step-text"><strong>Validate BitLocker posture</strong> — Confirm OS and fixed drives escrow recovery secrets to AD DS and that TPM + PIN enrollment succeeds on PAWs.</div></div>
<div class="step-card"><div class="step-number">5</div><div class="step-text"><strong>Verify Credential Guard and HVCI</strong> — Run <code>msinfo32</code> on PAW and confirm VBS: Running, Credential Guard: Running, Memory integrity enabled.</div></div>
<div class="step-card"><div class="step-number">6</div><div class="step-text"><strong>Test Remote Credential Guard</strong> — RDP from PAW to DC with <code>mstsc /remoteGuard</code> and verify SSO to network resources.</div></div>
<div class="step-card"><div class="step-number">7</div><div class="step-text"><strong>Plan WDAC rollout</strong> — Microsoft recommends App Control for Business on PAWs. Start with a base policy in audit mode, review Code Integrity events, then move to enforcement after validating the allowed software set.</div></div>
<div class="step-card"><div class="step-number">8</div><div class="step-text"><strong>Phase 6 — Create GPO Hardening Per Tier</strong> — Apply per-tier hardening baselines to DCs, T1 servers, T2 workstations.</div></div>
</div></div>

<p class="note" style="text-align:center;margin-top:2rem;">Generated by MATI — Phase 5 — Create PAW Hardening GPOs</p>
</div></body></html>
"@
