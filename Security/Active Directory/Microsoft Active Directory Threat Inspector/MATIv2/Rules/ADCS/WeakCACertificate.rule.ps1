...
@{
    ...
    Condition   = {
        ...
        foreach ($ca in $Data.CertificateServices.EnrollmentServices) {
            ...
            if ($ca.CertificateAlgorithm) {
                foreach ($weak in $weakAlgorithms) {
                    if ($ca.CertificateAlgorithm -match $weak) {
                        $issues += "WeakAlgorithm=$($ca.CertificateAlgorithm)"
                        break
                    }
                }
            }
            ...
            # Check key length
            if ($ca.KeyLength -and $ca.KeyLength -lt 2048) {
                $issues += "ShortKey=$($ca.KeyLength)bits"
            }
            ...
            if ($issues.Count -gt 0) {
                $findings += @{
                    ...
                    Details  = @{
                        CAName               = $ca.Name
                        CertificateAlgorithm = "$($ca.CertificateAlgorithm)"
