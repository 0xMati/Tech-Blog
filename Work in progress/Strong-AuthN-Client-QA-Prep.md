...
| 10 bis | C'est quoi ESS, et pourquoi l'historique PIN n'est plus mis en avant avec Windows 11 24H2 + VBS ? | **ESS = Enhanced Sign-in Security.** C'est un durcissement de la connexion Windows, appuyé par **VBS**, qui renforce la chaîne d'authentification locale (notamment avec biométrie compatible). Dans cette approche, Microsoft privilégie l'architecture passwordless moderne (**TPM, anti-hammering, device trust, CA**) plutôt que l'expiration/historisation PIN. En clair: **augmenter la complexité PIN et renforcer la posture de sécurité globale**, plutôt que compter sur la rotation des PIN. | Section 3.2 + Section 4.4 |
| 10 ter | C'est quoi ESS ? (réponse rapide) | **ESS (Enhanced Sign-in Security)** est un mode de durcissement de la connexion Windows qui protège davantage la chaîne de sign-in locale, en combinaison avec la sécurité matérielle et la posture device. | Section 3.2 |
| 10 quater | C'est quoi VBS ? (réponse rapide) | **VBS (Virtualization-Based Security)** est une technologie Windows qui isole des composants de sécurité dans un environnement virtualisé protégé, pour limiter la compromission du processus d'authentification et des secrets. | Section 3.2 + Section 4.4 |
| 11 | Comment fonctionne l'algorithme de complexité PIN dans WHfB ? | **Algorithme natif WHfB (non configurable):** bloque les PINs avec une **constant delta** (progression régulière entre chiffres). Exemples bloqués: 1111, 1234, 1357, 9630. Exemples autorisés: 1231 (delta 1,1,2), 1872 (delta 7,9,5). Cela élimine les suites simples, répétitions et patterns réguliers (100 PINs toujours bloqués). L'algorithme ne s'applique pas aux alphanumériques. | Politique WHfB |  
| 11 bis | Peut-on configurer des règles PIN supplémentaires via Intune/GPO ? | **Non.** La complexité PIN est une **caractéristique native de l'algorithme WHfB**, non configurable via GPO ou Intune. Les paramètres disponibles sont : longueur PIN, historique, expiration — mais pas les règles de pattern/delta. | Politique WHfB |
| 11 ter | Combien de tentatives échouées avant un blocage PIN ? (anti-hammering TPM) | **Par défaut: 3 tentatives échouées avant blocage 1 minute.** Après cumululation, blocage durable possible (max 32 tentatives erronées avant verrouillage). C'est un mécanisme **TPM anti-hammering**, pas une configuration GPO/Intune. | Section 3.2 TPM |
| 11 quater | Où trouve-t-on la documentation PIN reset ? | **Microsoft PIN reset service** - permet aux utilisateurs de récupérer un PIN Windows Hello oublié. Documentation: https://learn.microsoft.com/en-us/windows/security/identity-protection/hello-for-business/pin-reset | Runbook recovery |
